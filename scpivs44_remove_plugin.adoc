...
.Schritte
. Melden Sie sich in der SnapCenter VMware Plug-in Management GUI über das Format `\https://<OVA-IP-address>:8080`an.
+
...
. Klicken Sie im linken Navigationsbildschirm mit der rechten Maustaste auf die Instanz des SnapCenter-VMware-Plug-ins (den Namen der Datei, die bei der Bereitstellung der `.tar` virtuellen Appliance verwendet wurde), und wählen Sie *von Festplatte löschen* aus.
