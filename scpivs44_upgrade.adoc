...
. Laden Sie die Aktualisierungsdatei herunter `.iso` .
+
.. Melden Sie sich auf der NetApp Support-Website an (https://mysupport.netapp.com/products/index.html[]).
...
.. Laden Sie die Upgrade-Datei für das SnapCenter Plug-in für VMware vSphere an einen beliebigen Speicherort herunter `.iso` .
...
.. Navigieren Sie im Popup-Fenster „Hochladen“ zu dem Speicherort, an dem Sie die Datei heruntergeladen `.iso` haben, klicken Sie dann auf das `.iso` Dateibild und klicken Sie dann auf *Öffnen*. Die Datei wird in den Datastore hochgeladen.
...
+
... Wählen Sie in der Spalte Datastores den Datastore aus, in den Sie die Datei hochgeladen `.iso` haben.
... Navigieren Sie in der Spalte Inhalt zu der `.iso` hochgeladenen Datei, stellen Sie sicher, dass im Feld Dateityp „ISO-Image“ ausgewählt ist, und klicken Sie dann auf *OK*. Warten Sie, bis der Status „verbunden“ angezeigt wird.
